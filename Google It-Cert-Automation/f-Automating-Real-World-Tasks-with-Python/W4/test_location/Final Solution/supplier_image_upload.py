...
def verify_jpeg(file, source, ext):
    print(ext)
    if ext == ".jpeg":
        """ is it a jpeg? """
...
def main():
    ...
    if VERBOSE:
        print(data)
    for review in data:
        found_file = os.path.splitext(review)
        source = "{}{}".format(user_path, review)
        real_pic = verify_jpeg(review, source, found_file[1])
        if VERBOSE:
            source
        if not '.tiff' in review and STAGE_TWO:
