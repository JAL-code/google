...
def get_location_for_code():
    # :releative location
    access_area = '/home/joseph/google/Tkinter/DSP'
    last_folder = 'DSP'
...
def search_directory(home_address, rel_folder):
    ...
    """Get the selection files and duplicate them to the
    ...
    for root, dirs, files in os.walk(home_address):
        ...
        for file in files:
            ...
            if show_temp:
                print(f"Orginal: {found_file}| Copy To: {copy_location}")
            try:
...
def debug():
    verbose_state_1 = True
    show_temp_location = True
    copy_file = False
